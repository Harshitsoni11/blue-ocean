...
export class CourseService {
  ...
  /* MongoDB transactions are implemented in the code, but they are currently commented out since the application is running on a local standalone MongoDB instance. 
    Transactions in MongoDB are only supported on replica sets or mongos. 
    When deployed on MongoDB Atlas or a replica set environment, the transaction logic works as expected. */
  ...
  async create(dto: CreateCourseDto) {
    ...
    try {
      ...
      if (!isValid) {
        throw new BadRequestException({
          success: false,
          message: 'SubCategories must belong to selected Categories',
        });
      }
  ...
  async update(id: string, dto: UpdateCourseDto) {
    ...
    if (dto.subCategoryIds) {
      ...
      const selectedCategoryIds = categoryObjectIds.map((id) => id.toString());
